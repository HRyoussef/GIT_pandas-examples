...
sortRatingsField = sortRatingsField.groupby(['movie_id', 'title'])['rating'].agg(
    COUNT=np.size, myAVG=lambda x: x.sum() / float(x.count())).sort.values('COUNT', ascending=False)
print('My info sorted: \n%s' % sortRatingsField[:15])
=======
sortRF = cloneDF(mergeRatings)
sortRF = sortRF.groupby(['movie_id', 'title'])['rating'].agg(
    COUNT=np.size, myAVG=lambda x: x.sum() / float(x.count())).sort('COUNT', ascending=False)
print('My info sorted: \n%s' % sortRF[:15])
>>>>>>> e9ff759ed4317cfeba49e40bfe13240e85e1d6f8
